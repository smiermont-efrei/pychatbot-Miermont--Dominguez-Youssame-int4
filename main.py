...
def calculate_tf_idf_matrix_with_target_word(directory, president_names, tar-get_word):
...
def president_speaking_about_nation_the_most(tf_idf_matrix):
    ...
    for document in tf_idf_matrix:
        ...
        combined_counts[president] = combined_counts.get(president, 0) + tar-get_word_count

    max_count = max(combined_counts.values())
    presidents_with_max_count = [president for president, count in com-bined_counts.items() if count == max_count]
...
president_names = ['Chirac', 'Giscard d\'Estaing', 'Hollande','Mitterrand', 'Macron', 'Sarkozy']
target_word = 'Nation'
tf_idf_matrix_with_nation = calcu-late_tf_idf_matrix_with_target_word(speeches_directory, president_names, tar-get_word)
president_speaking_about_nation_the_most(tf_idf_matrix_with_nation)

# Fonction 10
def calculate_tf_idf_matrix_with_first_mention(directory, president_names, tar-get_words):
        ...
        for target_word in target_words:
            if target_word in mentions:
                ...
                if target_word not in first_mentions:
                    first_mentions[target_word] = (president, docu-ment['tf_idf_scores'][target_word])
...
tf_idf_matrix_with_first_mention = calcu-late_tf_idf_matrix_with_first_mention(speeches_directory, president_names, tar-get_words)
...
def words_mentioned_by_all_presidents(tf_idf_matrix, unimportant_words):
    ...
    for document in tf_idf_matrix[1:]:
        current_words = set(word for word, score in docu-ment['tf_idf_scores'].items() if score > 0)
...
tf_idf_matrix_without_unimportant = calcu-late_tf_idf_matrix_with_target_word(speeches_directory, president_names, 'Na-tion')
common_words = words_mentioned_by_all_presidents(tf_idf_matrix_without_unimportant, unim-portant_words)

print("Words Mentioned by All Presidents (Except Unimportant Words):", com-mon_words)
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            tf_idf_matrix = calcu-late_tf_idf_matrix_with_presidents(speeches_directory, president_names)
            most_repeated_words_by_president(tf_idf_matrix, 'Chirac')
        elif choice == '4':
            tf_idf_matrix_with_nation = calcu-late_tf_idf_matrix_with_target_word(speeches_directory, president_names, 'Na-tion')
            president_speaking_about_nation_the_most(tf_idf_matrix_with_nation)
        elif choice == '5':
            target_words = ['climate', 'ecology']
            tf_idf_matrix_with_first_mention = calcu-late_tf_idf_matrix_with_first_mention(speeches_directory, president_names, tar-get_words)
